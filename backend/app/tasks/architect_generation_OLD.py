    ...
    *,
    ...
    job_id = run_async(
        create_background_job(
            ...
            details={"run_id": run_id, "proposal_count": len(validated_proposals),},
        )
    )
    ...
    async with AsyncSessionMaker() as session:
        ...
        async with driver.session(database=settings.neo4j_database) as graph_session:
            ...
            existing_alias_map: dict[str, str] = {
                entity.alias.lower().strip(): entity.entity_instance_id
                for entity in ontology_instance.entities
                if entity.alias
            }
            existing_alias_details: dict[str, dict[str, Any]] = {
                entity.alias.lower().strip(): {
            ...
            try:
                ...
                created_alias_map: dict[
                    str, str
                ] = {}  # Map of normalized alias -> entity_instance_id
                created_alias_details: dict[str, dict[str, Any]] = {}
                ...
                if new_proposals:
                    ...
                    for proposal_id, entity_create in created_entities_map.items():
                        definition = entity_definitions_map.get(
                    ...
                    created_relationship_keys: set[tuple[str, int, str]] = set()
                    created_relationship_name_keys: set[tuple[str, str, str]] = set()
                    ...
                    for proposal_id in new_proposal_ids_ordered:
                        ...
                        proposal_entity_id_map[
                            proposal_id
                        ] = primary_entity.entity_instance_id
                        created_alias_map[
                            entity_create.alias.lower().strip()
                        ] = primary_entity.entity_instance_id
                        created_alias_details[entity_create.alias.lower().strip()] = {
                            "entity_instance_id": primary_entity.entity_instance_id,
                            "definition_id": primary_entity.definition_id,
                        }
                        created_entity_definition_map[
                            primary_entity.entity_instance_id
                        ] = primary_entity.definition_id
                    ...
                    for proposal_id in new_proposal_ids_ordered:
                        ...
                        rel_defs = {
                            rel["id"]: rel
                            for rel in entity_definitions_map[
                                entity_create.definition_id
                            ]["relationships"]
                        }
                        for rel_payload in relationships:
                            ...
                            if not target_id and target_alias:
                                ...
                                target_info = created_alias_details.get(
                                    normalized_alias
                                )
                                if target_info:
                                    target_id = target_info["entity_instance_id"]
                                    target_definition_id = target_info["definition_id"]
                                else:
                                    target_info = existing_alias_details.get(
                                        normalized_alias
                                    )
                                    if target_info:
                                        target_id = target_info["entity_instance_id"]
                                        target_definition_id = target_info[
                                            "definition_id"
                                        ]
                                    else:
                                        target_id = existing_alias_map.get(
                                            normalized_alias
                                        )
                                if target_id:
                                    logger.debug(
                                    ...
                                    target_result = await graph_session.run(
                                        ...
                                        RETURN e.entity_instance_id as eid, e.entity_definition_id as edid
                                        LIMIT 1
                                        """,
                                        ontology_id=run.ontology_id,
                                        alias=target_alias,
                                    ...
                                    if target_rec:
                                        ...
                                        target_definition_id = target_rec.get("edid")
                                        alias_key = target_alias.lower().strip()
                                        if alias_key:
                                            existing_alias_map[alias_key] = target_id
                                            existing_alias_details[alias_key] = {
                            ...
                            if destiny_entity_id:
                                if target_definition_id is None:
                                    if target_id in created_entity_definition_map:
                                        target_definition_id = created_entity_definition_map[
                                            target_id
                                        ]
                                    else:
                                        alias_key = (target_alias or "").lower().strip()
                                        target_definition_id = existing_alias_details.get(
                                            alias_key, {}
                                        ).get(
                                            "definition_id"
                                        )
                                if target_definition_id is None:
                                    logger.warning(
                                        "architect_generation: skipping relationship %s -> %s for definition %s expected target definition %s but target definition unknown",
                                        entity_create.alias,
                                        target_alias or target_id,
                                        rel_payload["definition_id"],
                                        destiny_entity_id,
                                    )
                                    continue
                                if target_definition_id != destiny_entity_id:
                                    logger.warning(
                                        "architect_generation: skipping relationship %s -> %s for definition %s expected target definition %s but got %s",
                                        entity_create.alias,
                                        target_alias or target_id,
                                        rel_payload["definition_id"],
                                        destiny_entity_id,
                            ...
                            if rel_key in created_relationship_keys:
                                logger.debug(
                                    "architect_generation: duplicate relationship skipped for %s -> %s def %s",
                                    entity_create.alias,
                                    target_alias or target_id,
                                    rel_payload["definition_id"],
                                )
                                continue
                            created_relationship_keys.add(rel_key)
                            name_key = (
                                source_entity_id,
                                (rel_def.get("name") or "").lower(),
                                target_id,
                            )
                            if name_key in created_relationship_name_keys:
                                logger.debug(
                                    "architect_generation: duplicate relationship name skipped for %s -> %s name %s",
                                    entity_create.alias,
                                    target_alias or target_id,
                                    rel_def.get("name"),
                                )
                                continue
                            created_relationship_name_keys.add(name_key)
                            ...
                            try:
                                ...
                                if rel_def.get("bi_directional"):
                                    ...
                                    if reverse_key not in created_relationship_keys:
                                        created_relationship_keys.add(reverse_key)
                                        await graph_session.run(
                                            """
                                            MATCH (source:EntityInstance {entity_instance_id: $source_id})
                                            MATCH (target:EntityInstance {entity_instance_id: $target_id})
                                            ...
                                            rel_id=str(uuid4()),
                                            rel_def_id=rel_payload["definition_id"],
                                            destiny_id=entity_create.definition_id,
                                            data=rel_data_json,
                                        )
                                logger.debug(
                ...
                if update_proposals:
                    ...
                    if missing_entity_ids:
                        ...
                        for entity_id in missing_entity_ids:
                            entity_result = await graph_session.run(
                                ...
                                RETURN e,
                                       collect({
                                           ...
                                           target_entity_instance_id: target.entity_instance_id
                                       }) as relationships
                                """,
                                entity_id=entity_id,
                            )
                            ...
                            if entity_record:
                                ...
                                rels = entity_record["relationships"]
                                ...
                                alias_key = (node.get("alias") or "").lower().strip()
                                if alias_key:
                                    existing_alias_map[alias_key] = node[
                                        "entity_instance_id"
                                    ]
                                    existing_alias_details[alias_key] = {
                                        "entity_instance_id": node[
                                            "entity_instance_id"
                                        ],
                                        "definition_id": node.get(
                                            "entity_definition_id"
                                        ),
                                    }
                            else:
                                logger.warning(
                                    "architect_generation: entity %s not found in graph, skipping",
                                    entity_id,
                                )
                    ...
                    for update_data in updates:
                        ...
                        for rel in update_data.get("new_relationships", []):
                            ...
                            target_definition_id: int | None = None
                            if target_id:
                                target_definition_id = created_entity_definition_map.get(
                                    target_id
                                ) or existing_entities_map.get(
                                    target_id, {}
                                ).get(
                                    "definition_id"
                                )
                            if not target_id and rel.target_alias:
                                ...
                                if target_id:
                                    ...
                                    if target_rec:
                                        target_id = target_rec["eid"]
                                        target_definition_id = target_rec.get("edid")
                                        alias_key = rel.target_alias.lower().strip()
                                        if alias_key:
                                            existing_alias_map[alias_key] = target_id
                                            existing_alias_details[alias_key] = {
                                                "entity_instance_id": target_id,
                            ...
                            if destiny_entity_id:
                                if target_definition_id is None:
                                    target_definition_id = existing_entities_map.get(
                                        target_id, {}
                                    ).get("definition_id")
                                if target_definition_id is None:
                                    logger.warning(
                                        "architect_generation: skipping update relationship for %s -> %s definition %s expected %s but target definition unknown",
                                        entity_id,
                                        rel.target_alias or target_id,
                                        rel.definition_id,
                                        destiny_entity_id,
                                    )
                                    continue
                                if target_definition_id != destiny_entity_id:
                                    logger.warning(
                                        "architect_generation: skipping update relationship for %s -> %s definition %s expected %s got %s",
                                        entity_id,
                                        rel.target_alias or target_id,
                                        rel.definition_id,
                                        destiny_entity_id,
                            ...
                            try:
                                ...
                                if rel_def.get("bi_directional"):
                                    ...
                                    if (
                                        reverse_name_key
                                        not in update_relationship_name_keys
                                    ):
                                        update_relationship_name_keys.add(
                                            reverse_name_key
                                        )
                                        await graph_session.run(
                                            """
                                            ...
                                            rel_id=str(uuid4()),
                                            rel_def_id=rel.definition_id,
                                            destiny_id=existing_entities_map.get(
                                                target_id, {}
                                            ).get("definition_id"),
                                            data=rel_data,
                                        )
