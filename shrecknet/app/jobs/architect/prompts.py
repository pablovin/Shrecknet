...
Core rule:
If this scene does not contain at least 2 meaningful milestones, return an empty milestone list.
Scenes with 0 milestones will be removed later.
...
ARCHITECT_PROPERTY_EXTRACTION_PROMPT = """You are extracting ontology data for a NEW entity.
...
Rules:
- Use only evidence present in the provided texts.
- Do not invent facts.
- Emit at most one value per property definition id.
- Emit relationships only when strongly justified and target alias is explicit.
- Use target_alias whenever possible; target_entity_instance_id can remain null.
- Keep autogenerated_summary concise and factual.

Return STRICT JSON only:
{{
  "properties": [
    {{"definition_id": 123, "value": "..."}}
  ],
  "relationships": [
    {{
      "definition_id": 456,
      "target_alias": "Some Alias",
      "target_entity_instance_id": null,
      "justification": "evidence-based reason"
    }}
  ],
  "autogenerated_summary": "2-4 sentences summary"
}}
"""


ARCHITECT_PROPERTY_UPDATE_PROMPT = """You are extracting UPDATE deltas for an existing entity.
...
Rules:
- Only include property updates when there is clear new evidence.
- Keep existing property values if there is no evidence of change.
- Propose relationships only when strongly justified.
- Keep updated_autogenerated_summary concise and factual.
- Do not invent facts.
...
{{
  ...
  "updated_autogenerated_summary": "updated summary text"
}}
"""
