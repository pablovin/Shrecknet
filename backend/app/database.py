...
def _migrate(conn):
    ...
    if "key_events" not in columns:
        conn.execute(text("ALTER TABLE page ADD COLUMN key_events JSON"))
    if "relationship_map" not in columns:
        conn.execute(text("ALTER TABLE page ADD COLUMN relationship_map JSON"))
    if "changelog" not in columns:
        conn.execute(text("ALTER TABLE page ADD COLUMN changelog JSON"))
