...
Summary: {summary}"""
    ...
    async def embed_ontology(
        ...
        for i in range(0, len(node_ids), batch_size):
            ...
            UNWIND $ids AS eid
            MATCH (n)
            WHERE n['entity_instance_id'] = eid
            ...
            OPTIONAL MATCH (before_event)
            WHERE 'Event' IN labels(before_event)
              AND before_event['event_id'] = n['before_event_id']
            OPTIONAL MATCH (after_event)
            WHERE 'Event' IN labels(after_event)
              AND after_event['event_id'] = n['after_event_id']
