...
@pytest.mark.asyncio
async def test_import_game_tables(
    client: AsyncClient, db_session: AsyncSession, admin_token: str
):
...
@pytest.mark.asyncio
async def test_import_sessions(
    client: AsyncClient, db_session: AsyncSession, admin_token: str
):
...
@pytest.mark.asyncio
async def test_import_full_workflow(
    client: AsyncClient, db_session: AsyncSession, admin_token: str
):
    ...
    print(
        f"✓ Users: {users_data['imported']} imported, {users_data['skipped']} skipped, {users_data['errors']} errors"
    )
    ...
    print(
        f"✓ Games: {games_data['imported']} imported, {games_data['skipped']} skipped, {games_data['errors']} errors"
    )
    ...
    print(
        f"✓ Sessions: {sessions_data['imported']} imported, {sessions_data['skipped']} skipped, {sessions_data['errors']} errors"
    )
...
async def test_import_idempotency(
    ...
    assert (
        data2["skipped"] == data1["imported"]
    ), "Second import should skip all previously imported users"
    print(
        f"✓ Idempotency test passed: {data2['skipped']} users skipped on second import"
    )
