...
async def generate_process_chunk_worker(
    ...
    system_prompt = (
        "You are an expert RPG campaign chronicler, that is helping me to mantain a wiki about my campaing sessions.\n"
        "I am sending you a part of my campaing session, novelized.\n"
        "I am sending you a list of pages that I want to create for my wiki.\n"
        "For EACH page below, read the session text, and use the provided instructions to generate:\n"
        ...
        "2. 'key_events': a list of events that this page is related to, if any, with event_type, event_date (if available), summary, and related_pages (by name, if any, from the text). Only generate events that are very important and that are referencing this page.\n"
        "3. 'relationships': a list of relationships that are related to each specific page, if any, with relationship_type, target_page (name from the text), direction (default 'outgoing'), and description. Return at most one relationship per target page and choose the most relevant if multiple are possible. Only do relationships that are explicitly mentioning this page.\n"
...
async def generate_process_source_worker(
    ...
    chunks = split_html_by_headers(sp.content or sp.autogenerated_content or "")
