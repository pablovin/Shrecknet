                    ...
                    if agent_entity_pairs:
                        ...
                        try:
                            for agent_id, agent_name, entity_id in agent_entity_pairs:
                                try:
                                    await update_job_progress(job_id, 0.92,
                                        {"status": f"Embodying {agent_name}"})
                                    svc = CharacterEmbodimentService(session, graph_session)
                                    await svc.append_created_scenes(
                                        agent_id=agent_id, entity_id=entity_id, ontology_id=ontology_id,
                                        created_scene_ids=created_scene_ids, llm_client=client, settings=settings)
