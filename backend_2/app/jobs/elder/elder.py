...
class ElderOrchestrator:
    ...
    async def execute(
        ...
        response = ElderQueryResponse(
            ...
            important_nodes=important_nodes if request.mode in ("context", "both") else [],
    ...
    async def _decompose(
        ...
        try:
            ...
            subqueries = [
                line.lstrip("0123456789.-) ") for line in lines if line
            ][: self.max_subqueries]
    ...
    async def _retrieve(
        ...
        results = await asyncio.gather(
            *[retrieve_with_limit(sq) for sq in subqueries]
        )
    ...
    async def _subanswer(
        ...
        async def answer_one(
            subquery: str, chunks: list[RetrievedChunk]
        ) -> SubAnswer:
        ...
        async def answer_with_limit(
            sq: str, chunks: list[RetrievedChunk]
        ) -> SubAnswer:
            async with semaphore:
    ...
    async def _validate(
        self, query: str, answer: str, trace: list[TraceStep]
    ) -> bool:
