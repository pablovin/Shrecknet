...
Summary: {summary}"""
    ...
    async def embed_ontology(
        ...
        MATCH (n)
          ...
          AND (
              ...
              OR n['last_updated_date'] > n['last_embedded_date']
                ...
                RETURN coalesce(n['entity_instance_id'], n['id']) AS node_id,
               CASE
        ...
        for i in range(0, len(entity_ids), batch_size):
            ...
            OPTIONAL MATCH (n)<-[:DERIVED_FROM]-(scene:Scene)
            WITH n, rels, collect(
                CASE
            ...
            OPTIONAL MATCH (n)<-[mrel:RELATES_TO]-(milestone:Milestone)
            OPTIONAL MATCH (milestone)<-[:CONTAINS]-(ms_scene:Scene)
