...
The Architect job analyses ontology instance narratives to propose new or updated entity instances for review.

#### Pipeline Steps

1. **Chunk** - Breaks existing entity text and autogenerated passages into coherent chunks
2. **Retrieve** - Surfaces likely matching EntityInstance nodes from Neo4j for grounding
3. **Extract** - Uses the Architect LLM prompt to emit JSON proposals for new instances and relevant existing ones
4. **Aggregate** - Deduplicates chunk suggestions and records evidence in `architect_proposals`

#### Outputs

- **Analysis Run** (`architect_analysis_runs`) recording the originating agent, ontology instance, and background job id
- **Proposals** (`architect_proposals`) containing status, confidence, justification, and supporting evidence for each suggestion

#### API Highlights

- `POST /jobs/architect/{agent_id}/analyze` — queue a step-one analysis against an ontology instance
- `GET /jobs/architect/runs/{run_id}` — retrieve run details and generated proposals
- `PATCH /jobs/architect/runs/{run_id}/proposals/status` — update proposal status (`pending`, `approved`, `rejected`)
