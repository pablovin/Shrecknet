...
import { BookOpenText, Search, Feather, ArrowLeftCircle } from "lucide-react";
import { useTranslation } from "../hooks/useTranslation";

const AGENT_PERSONALITIES = {
  "Lorekeeper Lyra":
    "“A tale untold is a world unseen. Let’s fill these pages with legend!”",
  "Archivist Axion":
    "“Every great saga begins with a single spark. Shall we kindle it together?”",
...
function AgentWriterPageContent() {
          ...
          <div className="min-h-screen w-full text-indigo-900 px-2 sm:px-6 py-12">
            <div className="mx-auto max-w-3xl flex flex-col gap-8 items-center">
              ...
              <div className="w-full flex flex-col items-center gap-4 bg-gradient-to-br from-indigo-100/70 via-fuchsia-100/80 to-white/80 rounded-2xl shadow-xl p-8 border border-indigo-200">
                ...
                <h1 className="text-3xl font-bold text-indigo-700 text-center font-serif mb-1">
                  {t("scribe_welcome")}
                </h1>
                ...
                <p className="text-center text-indigo-800/70 text-base">
                  {t("scribe_select")}
                  <br />
                  <span className="italic text-fuchsia-700">
                    {t("scribe_pen_line")}
                  </span>
                </p>
              </div>
                  ...
                  writerAgents.map((a) => (
                    ...
                    >
                      <Image
                        ...
                        width={100}
                        height={100}
                        className="rounded-full object-cover border-2 border-fuchsia-300 shadow mb-2"
                      />
                      <span className="text-xl font-bold text-indigo-800">
                        {a.name}
                      </span>
                      <span className="text-sm text-fuchsia-700">
                        {worldsMap[a.world_id]?.name}
                      </span>
  ...
  const agentWriterJobs = writerJobs.filter(
    (j) => j.agent_id === selectedAgent.id,
  );
  ...
  filtered.sort((a, b) => {
    ...
    if (sortField === "name") {
      ...
      va = conceptMap[a.concept_id]?.name || "";
      vb = conceptMap[b.concept_id]?.name || "";
    } else if (sortField === "autogenerated") {
      va = a.autogenerated_content ? 1 : 0;
      vb = b.autogenerated_content ? 1 : 0;
    } else {
      va = a.updated_at || "";
      vb = b.updated_at || "";
    }
        ...
        <div className="min-h-screen w-full text-indigo-900 px-2 sm:px-6 py-10">
          <div className="mx-auto max-w-5xl w-full flex flex-col gap-8">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 bg-gradient-to-br from-indigo-100/80 via-fuchsia-100/80 to-white/80 p-6 rounded-2xl shadow-xl border border-fuchsia-200">
              <Image
                src={selectedAgent.logo || "/images/default/avatars/logo.png"}
                alt={selectedAgent.name}
                width={160}
                height={160}
                className="w-40 h-40 rounded-full object-cover border-2 border-fuchsia-300 shadow"
              />
              <div className="flex-1 flex flex-col gap-1">
                <h2 className="text-2xl font-extrabold text-fuchsia-800 mb-1">
                  {selectedAgent.name}
                </h2>
                <span className="text-md text-indigo-700 mb-1">
                  {worldsMap[selectedAgent.world_id]?.name || ""}
                </span>
                <div className="italic text-fuchsia-600 mb-1">
                  {AGENT_PERSONALITIES[selectedAgent.name] ||
                    AGENT_PERSONALITIES.default}
                </div>
                <p className="mb-1">
                  I am your Scribe for{" "}
                  <span className="font-semibold">
                    {worldsMap[selectedAgent.world_id]?.name}
                  </span>
                  .<br />
                  Show me which stories, chronicles, or pages to read, and
                  together we’ll forge new legends for your world!
                </p>
              </div>
              <button
                onClick={() => {
                  ...
                  {recentJobs.map((j) => (
                    ...
                    >
                      <summary className="cursor-pointer flex justify-between p-1">
                        ...
                        <span>
                          {pageMap[j.page_id]?.name || j.page_names?.[0]}
                        </span>
                      </summary>
                      <div className="p-2">
                        ...
                        <div className="mt-2 flex gap-2">
                          ...
                          <Link
                            ...
                            href={`/agent_writer/${selectedAgent.id}/history/${j.job_id}`}
                          >
                            View
                          </Link>
                        </div>
            ...
            <div>
              <div className="flex flex-col sm:flex-row items-end gap-4 mt-10 mb-2">
                <div className="flex-1">
                  <h3 className="text-xl font-bold mb-1 text-fuchsia-700">
                    {t("library_of_lore")}
                  </h3>
                ...
                >
                  <Feather className="w-4 h-4 mr-1 inline" />{" "}
                  {t("process_selected")}
                </button>
              </div>
                ...
                <table className="min-w-full text-sm bg-white/80">
                  <thead>
                    <tr className="text-left text-fuchsia-800">
                      ...
                      <th
                        className="cursor-pointer"
                        onClick={() => changeSort("concept")}
                      >
                        Concept
                      </th>
                      <th
                        className="cursor-pointer"
                        onClick={() => changeSort("autogenerated")}
                      >
                        AI Content?
                      </th>
                      <th
                  ...
                  <tbody>
                    {paginated.map((p) => (
                      <tr
                        key={p.id}
                        className="border-b border-indigo-50 hover:bg-indigo-50 transition"
                      >
                        ...
                        <td className="py-2">
                          {conceptMap[p.concept_id]?.name || ""}
                        </td>
                        <td className="py-2 text-center">
                          {p.autogenerated_content ? "Yes" : "No"}
                        </td>
                        <td className="py-2">
                          {p.updated_at
