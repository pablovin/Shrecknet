...
@celery_app.task(name="backup.import_legacy_backup")
def import_legacy_backup_task(
    backup_path: str,
    ...
    job_id = existing_job_id or run_async(
        create_background_job(
            author_type=AuthorType(author_type),
            author_id=author_id,
            job_type=JobType.LEGACY_IMPORT,
            description=f"Importing legacy monolith backup {Path(backup_path).name}",
            celery_task_id=import_legacy_backup_task.request.id,
            details={
                "admin_user_id": admin_user_id,
                "backup_path": backup_path,
                "phase": "queued",
    ...
    try:
        ...
        run_async(report_progress("validating_archive", 0.05, "Validating uploaded archive"))

        sm = get_sessionmaker()
        with sm() as session:
            preserved_user = session.get(User, admin_user_id) if admin_user_id is not None else None

        if preserved_user is None:
            raise ValueError(f"Admin user {admin_user_id} not found")

        async def perform_import() -> dict[str, Any]:
            await report_progress("extracting_archive", 0.2, "Preparing legacy import")
            maintenance_service.enable("legacy_import")
            try:
                driver = get_driver()
                async with driver.session() as neo4j_session:
                    service = LegacyMonolithImportService()
                    await report_progress("database_restore", 0.45, "Importing Shrecknet SQLite data")
                    result = await service.import_backup_path(
                        Path(backup_path),
                        graph_session=neo4j_session,
                        preserved_user=preserved_user,
                    )
                    await report_progress("finalizing", 0.98, "Legacy import completed")
                    return result
            finally:
                maintenance_service.disable()

        result = run_async(perform_import())
        ...
        run_async(
            mark_job_done(
                ...
                details={
                    ...
                    "backup_path": backup_path,
                    "phase": "done",
                    "status": "Legacy import completed",
                    "restart_required": result.get("restart_required", False),
                    "result": result,
                },
            )
        ...
        logger.error("Failed to import legacy backup: %s", str(e), exc_info=True)
        run_async(mark_job_failed(job_id, f"[{current_phase}] {str(e)}"))
        raise
