...
class Ontology(Base):
    ...
    # Keep integer IDs for legacy compatibility across routers/services.
    ...
    rpg_system: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    ...
    world = relationship("World", back_populates="ontologies")
...
class OntologyProperty(Base):
    ...
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    image_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    cardinality: Mapped[Cardinality] = mapped_column(SqlEnum(Cardinality), nullable=False)
