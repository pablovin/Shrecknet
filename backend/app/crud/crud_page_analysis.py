...
async def generate_pages(
    ...
    ``page_specs`` should include for each page:

    - ``name`` and ``concept_id``
    - ``mode`` set to ``"create"`` or ``"update"``
    - ``target_page_id`` when ``mode`` is ``"update"``

    The function currently only handles pages with ``mode`` set to ``"create"``
    but the additional fields are accepted so that future implementations can
    update existing pages as well.

    Suggestions are grouped by their source pages so that each chunk of a source
    page is analysed only once. For every chunk the language model is asked to
    produce the ``autogenerated_content`` as well as any ``PageKeyEvent`` and
    ``PageRelationship`` for **all** pages associated with that source.
    """
