...
def upgrade() -> None:
    ...
    result = conn.execute(
        text("SELECT id, scheduled_time FROM session WHERE scheduled_time IS NOT NULL")
    )
    ...
    result = conn.execute(
        text(
            "SELECT id, proposed_time FROM sessionpolloption WHERE proposed_time IS NOT NULL"
        )
    )
    ...
    for option_id, proposed_time_str in options:
        if proposed_time_str:
            try:
                ...
                conn.execute(
                    text(
                        "UPDATE sessionpolloption SET proposed_time = :dt WHERE id = :id"
                    ),
    ...
    result = conn.execute(
        text("SELECT id, created_at FROM session WHERE created_at IS NOT NULL")
    )
    sessions = result.fetchall()
    ...
    result = conn.execute(
        text("SELECT id, created_at FROM sessionpoll WHERE created_at IS NOT NULL")
    )
    ...
    result = conn.execute(
        text('SELECT id, created_at FROM "table" WHERE created_at IS NOT NULL')
    )
