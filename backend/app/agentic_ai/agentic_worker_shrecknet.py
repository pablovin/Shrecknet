...
async def extract_metadata_and_chunks_worker(page: Page) -> Tuple[str, List[str]]:
    ...
    raw_content = page.content or page.autogenerated_content or ""
    page_chunks = split_html_by_headers(raw_content)
    chunk_texts = [
        ensure_visible_text(chunk) for chunk in page_chunks if chunk and chunk.strip()
    ]
