...
OLD_DATABASE_URL = os.getenv("OLD_DATABASE_URL", "sqlite+aiosqlite:///./data/prod.db")
    ...
    try:
        ...
        async with old_session_maker() as old_session:
            ...
            result = await old_session.execute(
                text("SELECT id, nickname, email, hashed_password, role, timezone FROM user")
            )
            ...
            logger.info(f"User import completed: {imported_count} imported, {skipped_count} skipped, {error_count} errors")
...
async def import_game_tables(
    ...
    try:
        ...
        async with old_session_maker() as old_session:
            ...
            result = await old_session.execute(
                text("SELECT id, name, world_id, crest_url, created_by, created_at FROM 'table'")
            )
            ...
            for old_table in old_tables:
                try:
                    ...
                    members_result = await old_session.execute(
                        text("SELECT user_id, is_gm FROM tablemember WHERE table_id = :table_id"),
                    ...
                    for old_member in old_members:
                        ...
                        if new_user:
                            ...
                            logger.info(f"Added member {new_user.username} to game '{old_table.name}'")
            ...
            await session.commit()
            logger.info(f"Game table import completed: {imported_count} imported, {skipped_count} skipped, {error_count} errors")
...
async def import_sessions(
    ...
    try:
        ...
        async with old_session_maker() as old_session:
            ...
            logger.info(f"Found {len(old_sessions)} sessions with scheduled dates in old database")
            ...
            for old_session_obj in old_sessions:
                try:
                    ...
                    if not new_game:
                        logger.warning(f"Game '{old_session_obj.table_name}' not found in new database, skipping session")
                        skipped_count += 1
                        continue
                    ...
                    if existing_session:
                        logger.info(f"Session '{old_session_obj.name}' already exists, skipping")
                        skipped_count += 1
                        continue
                    ...
                    attendance_result = await old_session.execute(
                        text("SELECT user_id, attending FROM sessionattendance WHERE session_id = :session_id"),
                    ...
                    for old_attendance in old_attendances:
                        ...
                        if new_user:
                            ...
                            logger.info(f"Added attendance for user {new_user.username} to session '{old_session_obj.name}'")
                    ...
                    error_count += 1
                    logger.error(f"Error importing session {old_session_obj.name}: {str(e)}")
            
            await session.commit()
            logger.info(f"Session import completed: {imported_count} imported, {skipped_count} skipped, {error_count} errors")
