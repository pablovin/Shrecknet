...
def migrate_agents_table(sync_conn) -> None:
    """Normalize legacy agents schema to the current app model."""
    ...
    if not needs_rebuild:
        ...
        sync_conn.execute(
            text("CREATE INDEX IF NOT EXISTS ix_agents_id ON agents (id)")
        )
        return
    ...
    try:
        sync_conn.execute(text("DROP TABLE IF EXISTS agents__migration"))
        sync_conn.execute(
            text(
                """
                CREATE TABLE agents__migration (
        ...
        sync_conn.execute(text("DROP TABLE agents"))
        sync_conn.execute(text("ALTER TABLE agents__migration RENAME TO agents"))
        sync_conn.execute(
            text("CREATE INDEX IF NOT EXISTS ix_agents_job ON agents (job)")
        )
        sync_conn.execute(
