...
async def generate_pages(
    ...
    """
    Create new pages based on suggestions, using LLM-extracted content, key events, and relationships.
    Events/relationships only link to existing or newly-created pages; otherwise, are discarded.
    All event dates default to today (UTC) if missing or invalid.
    """
    # 1. Separate specs by mode
    ...
    # 2. Fetch all necessary concepts/pages for reference and mapping
    ...
    # 3. Map each source page id to specs that reference it
    ...
    # Only needed when creating pages. Updates may not require sources.
    if create_specs and not sources_map:
        return {"pages": [], "updated": []}

    llm = ChatOpenAI(api_key=settings.openai_api_key, model=settings.open_ai_model)

    # 4. Prepare structure for collecting generated data
    spec_data: Dict[int, Dict[str, list]] = {
        id(spec): {"parts": [], "events": [], "rels": [], "spec": spec}
        for spec in create_specs
    }

    # 5. Async LLM extraction per source chunk
    async def process_chunk(sp: "Page", chunk: str, specs: List[dict]):
        ...
        for spec in specs:
            ...
            # Events: don't resolve names yet, just store as-is for now
            for ev in data.get("key_events", []) or []:
                ev["source_page_id"] = sp.id
                entry["events"].append(ev)
            # Relationships: store as-is (target_page by name)
    ...
    # 6. Create all new pages in DB and build new_name_to_page
    created_pages = []
    new_name_to_page = {}
    if create_specs:
        for sid, info in spec_data.items():
            ...
            gameworld_id = page_map[source_ids[0]].gameworld_id if source_ids else page.gameworld_id
            final_text = "\n\n".join(info["parts"])
            new_page = Page(
                name=spec["name"],
    ...
    # Combine old and new name-to-page maps for ID resolution
    ...
    for sid, info in spec_data.items():
        ...
        for rel_json in unique_rels:
            ...
            target_page = all_name_to_page.get(normalize_name(target_name)) if target_name else None
            if target_page:
                ...
                # Discard relationship if not resolvable
                print(f"[generate_pages] Discarded relationship without valid target_page_id: {rel}")
    ...
    if update_specs:
        ...
        for spec in update_specs:
            ...
            instructions = concept.auto_generated_prompt or "(no instructions, just summarize relevant content for this page)"
            sys_prompt = (
                "You are an expert RPG campaign chronicler.\n"
