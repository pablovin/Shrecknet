...
async def _resolve_previous_session_text(
    ...
    try:
        ...
        async with driver.session(database=settings.neo4j_database) as graph_session:
            ...
            # 3) Fallback to timeline events for instance-based session logs.
            timeline_result = await graph_session.run(
                """
                MATCH (:OntologyInstance {instance_id: $session_id})-[:HAS_TIMELINE_EVENT]->(event:TimelineEvent)
                RETURN event.description AS text,
                       event.description AS text_linked,
                       event.autogenerated_text AS autogenerated_text,
                       event.autogenerated_text AS autogenerated_text_linked
                ORDER BY coalesce(event.updated_at, event.created_at) DESC
                LIMIT 20
                """,
                session_id=session_id,
            )
            timeline_records = await timeline_result.data()
    except Exception:
        logger.warning(
    ...
    for record in timeline_records:
        text = _extract_best_text_from_record(record)
        if text:
            return text, "matched_ontology_instance_timeline"
