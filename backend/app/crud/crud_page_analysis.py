...
async def generate_pages(
    ...
    Only suggestions with ``mode`` set to ``"create"`` are processed. For each
    suggestion the content of the source pages is chunked by HTML headers and the
    language model is asked to produce an ``autogenerated_content`` following the
    concept's ``auto_generated_prompt``.
    """
    ...
    source_pages = {p.id: p for p in all_pages}

    async def process_spec(spec: dict) -> tuple[str, int] | None:
        concept = concept_map.get(spec["concept_id"])
        if not concept:
            return None

        source_ids = spec.get("source_page_ids", [])
        sources = [source_pages.get(pid) for pid in source_ids if source_pages.get(pid)]
        if not sources:
            return None

        # Gather header based chunks from all sources
        chunks: List[str] = []
        for sp in sources:
            chunks.extend(split_html_by_headers(sp.content or ""))
        ...
        async def summarize(chunk: str) -> str:
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        f"{concept.auto_generated_prompt or ''}\nExtract information about '{spec['name']}' from the text below. Respond with a short paragraph.",
                    ),
                    ("user", "{text}"),
                ]
            )
        ...
        summaries = await asyncio.gather(*(summarize(c) for c in chunks))
        ...
        combine_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    f"{concept.auto_generated_prompt or ''}\nCreate a single coherent description for '{spec['name']}' using the notes below.",
                ),
                ("user", "{notes}"),
            ]
        )
        chain = combine_prompt | llm
        combined = await chain.ainvoke({"notes": "\n".join(summaries)})
        final_text = combined.content.strip()
        ...
        return new_page.name, new_page.id

    generated = [r for r in await asyncio.gather(*(process_spec(s) for s in create_specs)) if r]
    return {"pages": [{"name": name, "id": pid} for name, pid in generated]}
