...
Each chronological `DERIVED_FROM` source group makes exactly six normal LLM
...
structured interpretations, and enrichment—but never reflections. Axis,
aspect, and goal updates then run in parallel.

The backend validates exact Scene order, stable impact targets, observation and
update evidence IDs, all eight axes, and configured list limits. Canonical Scene
...
All six system prompts document their complete input object and expected output
...
the third handles the three parallel persistent-profile update calls.
